...
def create_app(state: PiNOCState, config: Optional[Dict[str, Any]] = None, history: Any = None) -> Flask:
    ...
    def metrics(device_id):
        ...
        start=(datetime.now(timezone.utc)-__import__('datetime').timedelta(seconds=RANGES[name])).isoformat(); resolution="raw" if RANGES[name]<=7*86400 else "hourly"
        ...
        else:core=history.db.rows("SELECT bucket AS timestamp,avg_cpu AS cpu_percent,avg_temp AS cpu_temp_c,avg_memory AS memory_percent,max_cpu,max_temp,sample_count FROM metric_aggregates WHERE device_id=? AND resolution='hourly' AND bucket>=? ORDER BY bucket LIMIT 1000",(device_id,start))
        storage=history.db.rows("SELECT * FROM storage_metrics WHERE device_id=? AND timestamp>=? ORDER BY timestamp LIMIT 5000",(device_id,start));network=history.db.rows("SELECT * FROM network_metrics WHERE device_id=? AND timestamp>=? ORDER BY timestamp LIMIT 5000",(device_id,start))
