...
// strong actions (reboot, shutdown, service stop) require confirmation first.
const runbookGate=(action,deviceId,resource)=>action==='device.reboot'?{type:'confirm',text:`Reboot ${deviceId}?`}:action==='device.shutdown'?{type:'prompt',match:deviceId,text:`Type ${deviceId} to confirm shutdown`}:action==='service.stop'?{type:'prompt',match:resource,text:`Type ${resource} to confirm stopping it on ${deviceId}`}:null;
