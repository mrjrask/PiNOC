...
def parse_smart(value):
    x=json.loads(value) if isinstance(value,str) else value; passed=(x.get("smart_status") or {}).get("passed"); attrs={a.get("name"):a.get("raw",{}).get("value") for a in (x.get("ata_smart_attributes") or {}).get("table",[])}
