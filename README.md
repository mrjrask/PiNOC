...
environment must be created with Debian's `/usr/bin/python3` and expose system
site packages just like the environment created by `install.sh`. This explicit
path avoids accidentally creating the environment with pyenv, Conda, or another
Python installation that cannot see Debian packages.
