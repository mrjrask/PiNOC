...
def test_schema_enrollment_replay_rotation_and_revocation(tmp_path):
 db,gw=setup(tmp_path);assert SCHEMA_VERSION==10;a=enroll(gw);body=b'{}';stamp=str(int(time.time()));nonce="unique";sig=gw.sign(a["agent_id"],a["credential"],stamp,nonce,body)
